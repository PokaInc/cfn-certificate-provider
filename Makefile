...
S3_BUCKET_NAME=$(shell aws cloudformation describe-stacks --stack-name cfn-certificate-provider-code-bucket --query "Stacks[0].Outputs[?OutputKey=='CfnCertificateProviderCodeBucket'].OutputValue" --output text --region $(AWS_REGION))
ALL_REGIONS=$(shell printf "import boto3\nprint('\\\n'.join(map(lambda r: r['RegionName'], boto3.client('ec2').describe_regions()['Regions'])))\n" | python | grep -v '^$(AWS_REGION)$$')

help:
	@echo 'make                 - builds a zip file to target/.'
	@echo 'make deploy-code     - builds a zip file and deploys it to Poka's private S3 bucket.'
...
deploy-code-bucket:
	aws cloudformation deploy --stack-name cfn-certificate-provider-code-bucket \
		--template-file cloudformation/code-bucket.yaml

deploy-cfn-certificate-provider:
	aws cloudformation deploy --stack-name cfn-certificate-provider \
		--template-file cloudformation/cfn-resource-provider.yaml \
		--capabilities CAPABILITY_IAM

deploy-code: build target/$(NAME)-$(VERSION).zip
	aws s3 --region $(AWS_REGION) \
		cp target/$(NAME)-$(VERSION).zip \
		s3://$(S3_BUCKET_NAME)/lambdas/$(NAME)-$(VERSION).zip
	aws s3 --region $(AWS_REGION) cp \
		s3://$(S3_BUCKET_NAME)/lambdas/$(NAME)-$(VERSION).zip \
		s3://$(S3_BUCKET_NAME)/lambdas/$(NAME)-latest.zip
